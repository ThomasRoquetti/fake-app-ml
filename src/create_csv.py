...
header = ["indice","author", "link", "category", "date of publication", "number of tokens", "number of words without punctuation", 
...
with open('test.csv', 'w', newline='') as f_output:
    ...
    for x in range(1, 3603):
        # try:
            filepath = os.path.normpath(r"/home/thoborges/fake-app-ml/fake-corpus-br/full_texts/{}-meta-information/{}-meta.txt".format('fake', x))

            with open(filepath, 'r', newline='') as f_text:
                with open(filepath, 'a') as f_text_indice:
                    f_text_indice.write(str(x))
